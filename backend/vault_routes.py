...
async def get_vault_status(_session: AuthSession = Depends(require_dm)):
    return VaultStatusResponse(providers=vault_status())


@router.post("/vault/store")
async def store_key(data: VaultStoreRequest, _session: AuthSession = Depends(require_dm)):
    store_api_key(data.provider, data.api_key)
    return {"status": "ok", "provider": data.provider}


@router.post("/vault/delete")
async def delete_key(data: VaultDeleteRequest, _session: AuthSession = Depends(require_dm)):
